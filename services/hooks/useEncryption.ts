...
export function usePasswordEncryptor() {
  ...
  const encryptPassword = useCallback(
    async (plain: string, publicKeyPem: string) => {
      try {
        setLoading(true);
        setError(null);
...
export const fetchEncryptionKey = async (): Promise<EncryptionKeyResponse> => {
  const response = await get<EncryptionKeyResponse>("/auth/encryption-key");
  return response;
};

export const useEncryptionPublicKey = () => {
  return useQuery({
    queryKey: ["public-key"],
